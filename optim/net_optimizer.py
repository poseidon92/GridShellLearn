...
from utils.utils import save_mesh, save_cloud, map_to_color_space, export_vector
...
class StructuralNetOptimizer:
    ...
    def __init__(self, file, lr, momentum, device, loss_type, no_knn, transform_in_features, get_loss, layer_mode):
        ...
        self.start_loss = self.Structural_calculus(self.initial_mesh, self.loss_type)
        self.lr = lr
    ...
    def optimize(self, n_iter, save, save_interval, display_interval, save_label, take_times, neighbor_list, save_prefix=''):
        ...
        if save:
            ...
            export_vector(self.Structural_calculus.beam_energy, save_prefix + '[START]energy_' + save_label + '.csv')
            export_vector(map_to_color_space(self.Structural_calculus.beam_energy.cpu(), vmin=0, vmax=vmax), save_prefix + '[START,RGBA]energy_' + save_label + '.csv', format='%d')
        ...
        for current_iteration in range(n_iter):
            ...
            offset[self.Structural_calculus.non_constrained_vertices, :] = displacements[self.Structural_calculus.non_constrained_vertices]
            ...
            if current_iteration % save_interval == 0:
                if save:
                    ...
                    export_vector(map_to_color_space(self.Structural_calculus.beam_energy.detach().cpu(), vmin=0, vmax=vmax), filename, format='%d')
            ...
            if structural_loss < best_loss:
                ...
                if save:
                    ...
                    best_displacements = torch.norm(self.Structural_calculus.vertex_deformations[:, :3], p=2, dim=1)
